...
dateInput.addEventListener("input", () => {
  function dateIn() {
    if (dateInput.value.length < 8) {
      dateInput.value.length.slice(0, 2) +
        "/" +
        dateInput.value.length.slice(2);
    }
  }
});
...
function getInfo() {
  ...
  credit.forEach((el, index) => {
    ...
    del.innerHTML = '<ion-icon name="trash-bin-outline"></ion-icon>';
...
function numInput() {
  if (numberInput.length > 4 && numberInput.length < 16) {
    numberInput.value.slice(0, 4) +
      "-" +
      numberInput.value.slice(5, 9) +
      "-" +
      numberInput.value.length(10, 14) +
      "-" +
